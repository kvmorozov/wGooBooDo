 ...
 */
class ShplImageExtractor(bookContext: BookContext) : AbstractImageExtractor(bookContext, ShplImageExtractor::class.java) {
    ...
    private inner class EventProcessor internal constructor(private val proxy: HttpHostExt) : Runnable {
        ...
        override fun run() {
            ...
            for (page in uniqueObject.bookInfo.pages.pages)
                uniqueObject.imgExecutor.execute(SimplePageImgProcessor(uniqueObject, page as ShplPage, HttpHostExt.NO_PROXY))
