...
public class WindowsShortcut {
    ...
    private static boolean isMagicPresent(byte[] link) {
        int magic = 0x0000004C;
        int magic_offset = 0x00;
    ...
    private void parseLink(byte[] link) throws ParseException {
        try {
            ...
            int file_atts_offset = 0x18;
            byte file_atts = link[file_atts_offset];
            byte is_dir_mask = (byte) 0x10;
            this.isDirectory = 0 < ((int) file_atts & (int) is_dir_mask);

            // if the shell settings are present, skip them
            int shell_offset = 0x4c;
            byte has_shell_mask = (byte) 0x01;
            int shell_len = 0;
            ...
            int file_location_info_flag_offset_offset = 0x08;
            int file_location_info_flag = link[file_start + file_location_info_flag_offset_offset];
            this.isLocal = 0 == (file_location_info_flag & 2);
            // get the local volume and local system values
            //final int localVolumeTable_offset_offset = 0x0C;
            int basename_offset_offset = 0x10;
            int networkVolumeTable_offset_offset = 0x14;
            int finalname_offset_offset = 0x18;
