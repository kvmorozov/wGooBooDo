...
//            HttpsURLConnection.setDefaultHostnameVerifier(allHostsValid)
            ...
            val reqBuilder = HttpRequest.newBuilder()
                .uri(baseUrl).GET().timeout(Duration.ofMillis(10000))
                .setHeader("User-Agent", USER_AGENT)
                .setHeader("Cookie", "CONSENT=PENDING+999")

//            val con: URLConnection = baseUrl.toURL().openConnection()
            ...
            if (GBDOptions.debugEnabled) {
                logger.info("SSL error for $proxy : ${ssle.message}")
            }
